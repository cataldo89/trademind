...
test('Screener client implements candles fallback and noData handling', () => {
  ...
  assert.match(scanRoute, /\(quality\.usable_for_ml \|\| prelim\.recentIpoFallback\) && sent\.sentiment === 'POSITIVE'/)
  ...
  assert.match(ranking, /preliminary\.recentIpoFallback/)
  assert.match(ranking, /recentIpoFallback/)
  assert.match(ranking, /indicatorMode/)
  assert.match(scanRoute, /recent_ipo_fallback/)
  assert.match(scanRoute, /LightGBM long-term indicators disabled/)
...
test('Screener blocks LightGBM ready state when asset ranking is fallback', () => {
  ...
  assert.match(client, /Ranking tecnico rapido \(fallback\)/)
  ...
  assert.match(client, /const detailedScanSymbols = normalizedSearch/)
  assert.match(client, /const shouldRunDetailedScan = detailedScanSymbols\.length > 0/)
  assert.match(client, /detailedScanSymbols\.map\(\(s\) => s\.symbol\)/)
  assert.match(client, /&& forceQuantRefreshNonce > 0/)
  assert.match(client, /&& forceQuantRefreshCategory === category/)
  assert.match(client, /setForceQuantRefreshCategory\(category\)/)
  assert.match(client, /enabled:\s*shouldRunDetailedScan/)
  ...
  assert.match(assetRankRoute, /applyCryptoPolicyToRankings/)
  assert.match(assetRankRoute, /crypto_policy_blocks_lightgbm/)
  assert.match(assetRankRoute, /local_recent_ipo_fallback/)
  assert.match(assetRankRoute, /recent_ipo_short_history/)
  assert.match(assetRankRoute, /50 \+ \(return1d \* 125\)/)
  assert.match(assetRankRoute, /recentIpo\.score >= 62/)
  assert.match(assetRankRoute, /history_candles/)
  assert.match(marketData, /symbol:\s*'FLHY',\s*name:\s*'Franklin High Yield Corporate ETF'/)
  assert.doesNotMatch(marketData, /symbol:\s*'FLHK'/)
})
