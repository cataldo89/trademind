...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      console.error('[api/profile/capital GET]', error)
    ...
    console.error('[api/profile/capital GET fatal]', error)
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (updateError) {
      ...
      console.error('[api/profile/capital PATCH]', updateError)
      return NextResponse.json({ error: 'Failed to update virtual balance' }, { status: 500 })
    }
    ...
    if (profileError) {
      ...
      console.error('[api/profile/capital profile upsert]', profileError)
      return saveBalanceInAuthMetadata(dbClient, user, virtualBalance)
    }

    return profileResponse(parseVirtualBalance(profileData?.virtual_balance), 'profiles')
  } catch (error) {
    console.error('[api/profile/capital PATCH fatal]', error)
    return NextResponse.json({ error: 'Unexpected profile error' }, { status: 500 })
  }
}